...
NyTT Classifier with DUNet and Multi-view pooling.

Run
    python -m denoising.nytt.model.train_v4 --fold 1 \
        --dilations 1,2,8,32 --stft_fmax_hz 2000 --w_stft 0.7
"""
...
UTILS_ROOT = CURRENT_DIR.parents[2]  # /home/coder/workspace/data/classification
...
def get_args():
    ...
    p.add_argument("--model_name", default="dnet_multiview_poolfusion")
...
def stage_check(args):
    print("=" * 78)
    _dil = tuple(int(d) for d in str(args.dilations).split(",") if d.strip())
    for predict in ("signal", "residual"):
        for use_skip in (True, False):
            model = DUNet(kernel=args.kernel, stride=args.stride,
    ...
    fe = LogMelExtractor(normalize=args.mel_normalize, top_db=args.top_db)
    with torch.no_grad():
    ...
    if mel_path.exists():
        from data.data_loader import preprocessing_mel
    ...
    if Path(args.noise_bank).exists():
        ...
        verify_fold_noise_bank(
            args.noise_bank, args.noise_fold, noise_train_ids, noise_test_ids,
        )
        nb = NoiseBank(args.noise_bank)
        excluded_sources, excluded_clips = exclude_eval_sources(nb, ev.sample_ids)
        if excluded_sources:
            print(f"[noise] data fold {args.fold} test 누수 방지: "
                  f"source {excluded_sources} ids / {excluded_clips} clips 제외")
        ...
        print(
            "\n[noise] 없음 — python -m denoising.nytt.noise_bank.noise_bank_split "
            f"--fold {args.noise_fold} 를 먼저 실행하세요."
        )

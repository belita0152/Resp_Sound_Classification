...
"""
NyTT v4 training for the ICBHI 2017 dataset.

Run
    python -m denoising.nytt.model.train_v4_icbhi --fold 1
"""
...
from data.icbhi_dataloader import LABEL_NAMES
from data.icbhi_parser import (FOLD_MANIFEST_PATH, ICBHI_AUDIO_DIR,
                               ICBHI_DERIVED, MANIFEST_PATH, MEL_FOLDER, WAVE_FOLDER)
from denoising.nytt.model.utils import (
    FOLD_SPLIT_COL,
    build_icbhi_datasets,
...
DEFAULT_SPLIT_MANIFEST = FOLD_MANIFEST_PATH
DEFAULT_NOISE_BANK_DIR = ICBHI_DERIVED
    ...
    def eval_one_epoch(self, epoch: int, tag: str | None = None):
        ...
        if getattr(self, "_dist_on", False) and dist_prof:
            front = (self.core.view_frontends[0]
                     ...
                     else self.core.frontend)
            fb = front.fb
            freqs = (fb.argmax(dim=1).cpu().numpy()
                     * (SR / (2 * (fb.shape[1] - 1))))
            plot_group_distribution(
                torch.cat(dist_prof).numpy(), freqs, y_true, y_pred,
                self.class_names,
...
def get_args():
    ...
    p.add_argument("--wave_base_path", default=str(WAVE_FOLDER),
                   help="ICBHI 10s_repeat 폴더. --icbhi_wave_dir 와 같은 값이면 된다")
    p.add_argument("--fold", default=1, type=int, choices=[1, 2],
                   help="1=ICBHI 공식 60/40 (train_test.txt, 환자 2명 겹침) / "
                        "2=환자 분리 재분할 (누수 0)")
    ...
    p.add_argument("--split_manifest", default=str(DEFAULT_SPLIT_MANIFEST),
                   help="fold,sample_id,split 열을 가진 CSV. 없으면 manifest 에서 생성한다")
    p.add_argument("--noise_bank", default=None,
                   help="기본값: <ICBHI>/derived/noise_bank_icbhi_fold<NOISE_FOLD>.npz")
    p.add_argument("--model_name", default="dnet_v4_icbhi")
    p.add_argument("--save_dir", default=None,
                   help="기본값: denoising/nytt/ckpt/icbhi_fold<FOLD>")
    p.add_argument("--drop_both", action="store_true",
                   help="Both(3) 를 제외해 3-class 로 만든다. --num_classes 도 따라간다")
    p.add_argument("--min_sr", default=None, type=int,
                   help="이 미만 sampling rate 의 녹음을 제외한다. 8000 이면 "
                        "4 kHz 녹음 88개(Nyquist 2 kHz)가 빠져 798 rec / 5,642 cycles 가 남는다")
    p.add_argument("--icbhi_devices", default="",
                   help="콤마 구분 기기 필터. 예: AKGC417L "
                        "(AKGC417L 620 / Meditron 121 / LittC2SE 86 / Litt3200 59 recordings)")
    p.add_argument("--icbhi_manifest", default=str(MANIFEST_PATH),
                   help="icbhi_parser.py 가 만든 cycle manifest CSV")
    p.add_argument("--icbhi_wave_dir", default=str(WAVE_FOLDER),
                   help="icbhi_preprocess.py 의 10s_repeat 폴더")
    p.add_argument("--icbhi_mel_dir", default=str(MEL_FOLDER),
                   help="icbhi_preprocess.py 의 10s_mel 폴더")
    p.add_argument("--icbhi_audio_dir", default=str(ICBHI_AUDIO_DIR),
                   help="원본 ICBHI wav 폴더. manifest 의 절대경로가 이 머신과 "
                        "다를 때 여기 기준으로 다시 잡는다")
    ...
    p.add_argument("--aux_ce_weight", default=0.0, type=float,
                   help="호환용 인자. v4는 auxiliary logits를 만들지 않아 적용되지 않음")
    ...
    p.add_argument("--distribution", action="store_true",
                   help="최종 test 1회 평가에서 N->N / W->N / W->W 세 군의 주파수 "
                        "에너지 분포를 그림 하나로 저장한다 "
                        "(<save_dir>/<model_name>_distribution.png)")
    ...
    if args.noise_fold not in FOLD_SPLIT_COL:
        p.error(f"--noise_fold 는 {sorted(FOLD_SPLIT_COL)} 중 하나여야 합니다.")
    if args.aux_ce_weight < 0:
        p.error("--aux_ce_weight는 0 이상이어야 합니다.")
    expected = 3 if args.drop_both else 4
    if args.num_classes is None:
        args.num_classes = expected
    elif args.num_classes != expected:
        p.error(f"ICBHI 는 --drop_both {'있으면 3' if args.drop_both else '없으면 4'}-class "
                f"입니다. 받은 --num_classes {args.num_classes}")
    if args.noise_bank is None:
        args.noise_bank = str(
            DEFAULT_NOISE_BANK_DIR / f"noise_bank_icbhi_fold{args.noise_fold}.npz"
        )
    if args.save_dir is None:
        args.save_dir = str(CURRENT_DIR.parent / "ckpt" / f"icbhi_fold{args.fold}")
    return args
